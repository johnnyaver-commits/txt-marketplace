...
import { listingToCardProps } from '@/modules/listings/view-model';

export default function ListingsPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  void searchParams;
  const cards = mockListings.map(listingToCardProps);
      ...
      <section className="mx-auto max-w-7xl py-12">
        <div className="mb-8 flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl leading-7 text-[#6f6677]">先用精選 mock 商品呈現 UI；接上 Neon PostgreSQL 後，此頁會改讀 `/api/listings` 真實資料。</p>
          </div>
